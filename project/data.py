...
class CityScapesDataset(Dataset):
    def __init__(self, root_dir, transform):
        ...
        self.bbox_dir = root_dir + "/bbox_v2"
    ...
    def __getitem__(self, index):
        real = Image.open(self.real[index])
        real = self.transform(real)
        noise = Image.open(self.noise[index])
        noise = self.transform(noise)
        f = open(self.bboxes[index])
        bbox_dict = json.load(f) # za učitati info iz bbox datoteke
        f.close()
        bbox = [bbox_dict['x'], bbox_dict['y'], bbox_dict['w'], bbox_dict['h']]

        return {'real': real, 'noise': noise, 'bbox': bbox}
...
class Cityscapes(pl.LightningDataModule):
    def __init__(self, data_dir="./data", batch_size=1):
        ...
        # možda i Normalize?? u tom slučaju:
    ...
    def setup(self, stage=None):
        # self.data = CityScapesDataset(root_dir=self.data_dir)
        if stage == 'fit' or stage is None:
            self.train = # novi folder sa slikama za trening
        if stage == 'test' or stage is None:
            self.test = # novi folder sa slikama za test
