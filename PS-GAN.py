# izvor: https://github.com/yueruchen/Pedestrian-Synthesis-GAN/blob/master/models/networks.py

# PITANJA: l.60. PADDING NIJE NUŽNO CIJELI BROJ??
#          l.147. ODNOSI SE GPU NA MENE? (AWS...)
#          l.186. UNet blokovi i skip konekcije?
...
# podaci!!!, inicijalizacija tezina, use_gpu

class PersonDiscriminator(nn.Module): # no ls_gan = False
    def __init__(self, gpu_ids=[]): # ndf = broj diskriminatorovih filtera
    ...
    def spatial_pyramid_pool(self, previous_conv, num_sample, previous_conv_size, out_pool_size):
        ...
        for i in range(len(out_pool_size)):
            ...
            h_pad = (h_wid*out_pool_size[i] - previous_conv_size[0] + 1)/2 # NIJE NUŽNO CIJELI BROJ????????????????????????????????????????????????????
...
class BackgroundDiscriminator(nn.Module): #no ls_gan = true
    ...
    def forward(self, input):
        if len(self.gpu_ids) and isinstance(input.data, torch.cuda.FloatTensor): # odnosi se ovo na mene???? 
